...
namespace Cars_Racing.Vehicle.EngineLogic
{
    public class Engine : MonoBehaviour
    {
        ...
        private void Gas(float gasValue)
        {
            ...
            CalculatePotentialRPM(gasValue);
            ...
            _information = $"Speed: {CurrentSpeed}, RPM: {CurrentRPM}, Potential RPM: {PotentialRPM}, Motor Toqgue: {MotorTorgue}";
        }
        ...
        private float CalculateMotorTorque(float gasValue)
        {
            ...
            float currentGearMinSpeed = !_gearbox.IsNeutralGear ? (avgGearSpeed * (_gearbox.CurrentGear - 1) * 1.2f) : 0;
            ...
            motorTorque = gasValue * rpmGearPower * torque * gearMultiplier;

            return motorTorque;
        }

        private void CalculatePotentialRPM(float gasValue)
        {
            PotentialRPM = Mathf.Clamp(PotentialRPM + Mathf.Lerp(-1,1, Mathf.Abs(gasValue) * (CarConfigurationInfo.CarConfiguration.HorsePower * 745.7f) / ((2 * Mathf.PI * CurrentRPM) / 60) * Time.deltaTime), CarConfigurationInfo.CarConfiguration.MinRPM, CarConfigurationInfo.CarConfiguration.MaxRPM);
        }
    }
}